...
## ==== Tab 1
## ==========
data_tab2_scatter <- deposit_details %>%
...
data_tab2_scatter_country_colours <- data_frame(
  country = unique(data_tab2_scatter$country),
  colour = col_vector[1:length(unique(data_tab2_scatter$country))]
...
data_tab1_linechart <- deposit_details %>%
  filter(name == "data_iframe_4b.csv") %>%
  select(download_url) %>%
  .[[1]] %>%
  read_csv()

data_tab1_linechart_country_colours <- data_frame(
  country = unique(data_tab1_linechart$country),
  colour = col_vector[1:length(unique(data_tab1_linechart$country))]
  # colour = unlist(lapply(col_vector[1:length(unique(data_tab1_6states$country))], function(x) add.alpha(x, 0.1)))
  # hover.colour = unlist(lapply(col_vector[1:length(unique(data_tab1_6states$country))], function(x) add.alpha(x, 1)))
  # hover.colour = rgb(1, 1, 1, 0.8)
)
...
data_tab1_linechart <- data_tab1_linechart %>%
  mutate(colour = mapvalues(country, from = data_tab1_linechart_country_colours$country, to = data_tab1_linechart_country_colours$colour)
         # hover.colour = mapvalues(country, from = country_colours$country, to = country_colours$hover.colour)
  )

female_data <- data_tab1_linechart %>%
  filter(female == 1) %>%
  select(-contains("male")) %>%
  rename(female_standardized_report_per_round = standardized_report_per_round)

male_data <- data_tab1_linechart %>%
  filter(female == 0) %>%
  select(-contains("male")) %>%
  rename(male_standardized_report_per_round = standardized_report_per_round)

data_tab1_linechart <- full_join(
  male_data,
  female_data,
