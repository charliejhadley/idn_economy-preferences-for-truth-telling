observeEvent(input$tab1_linechart_reset, {
  shinyjs::reset("tab1_linechart_controls")
})

output$tab1_linechart_selected_countries_UI <- renderUI({
  available_countries <- data_tab1_linechart %>%
    select(country) %>%
    .[[1]] %>%
    unique()
  ...
  selectInput(
    "tab1_linechart_selected_countries",
...
output$tab1_linechart_citation_selectize_UI <- renderUI({
  if (is.null(input$tab1_linechart_selected_countries)) {
    available_citations <- data_tab1_linechart %>%
      select(citation) %>%
      .[[1]] %>%
      unique()
  } else {
    available_citations <- data_tab1_linechart %>%
      filter(country %in% input$tab1_linechart_selected_countries) %>%
      select(citation) %>%
      .[[1]] %>%
      unique()
  }
  ...
  selectInput(
    "tab1_linechart_citation_selectize",
...
data_tab1_linechart_hc <-
  eventReactive(
    c(
      input$tab1_linechart_population_options,
      input$tab1_linechart_citation_selectize,
      input$tab1_linechart_repeated_or_oneshot,
      input$tab1_linechart_location,
      input$tab1_linechart_controls_suggested,
      input$tab1_linechart_draw_or_mind,
      input$tab1_linechart_selected_countries
    ),
    {
      if (is.null(input$tab1_linechart_selected_countries)) {
        data_tab1_linechart <- data_tab1_linechart
      } else {
        data_tab1_linechart <- data_tab1_linechart %>%
          filter(country %in% input$tab1_linechart_selected_countries)
      }
      ...
      if (nrow(data_tab1_linechart) == 0) {
        # no data
        return(data_frame())
      } else {
        if (!is.null(input$tab1_linechart_citation_selectize)) {
          data_tab1_linechart <- data_tab1_linechart %>%
            filter(citation %in% input$tab1_linechart_citation_selectize)
        }
      }
